##  Adapted from https://github.com/tomgoldstein/pytorch-wavenet
...
class WaveNetModel(nn.Module):
    ...
    Args:
        ...
        fist_layer_weight_share:    If True, use same weights for each channel in input layer
    ...
    def __init__(self,
        ...
        self.dtype = dtype
        self.fist_layer_weight_share = fist_layer_weight_share
        ...
        if fist_layer_weight_share:
            self.start_conv = nn.Conv1d(in_channels=1,
                                        out_channels=residual_channels/in_channels,
                                        kernel_size=2,
                                        bias=bias)
        else:
    ...
    def wavenet(self, input, dilation_func, verbose=False):
        if self.fist_layer_weight_share:
            #input has shape batch x 8 x 100000
            b, c, n = input.shape
            input = input.view(b*c,1,n)   # treat each input channel as an identical input
            x = self.start_conv(input)
            x = x.view(b,self.residual_channels,-1)
        else:
            x = self.start_conv(input)

        skip = 0

        self.print_verbose(f'start_conv_out = {x.shape}')
        ...
        for i in range(self.blocks * self.layers):
            ...
            #self.print_verbose(f'resid{i}_out = {residual.shape}')
            # dilated convolution
            ...
            #self.print_verbose(f'x{i}_out = {x.shape}')
            ...
            #self.print_verbose(f'skip{i}_out = {skip.shape}')

            x = self.residual_convs[i](x)
            #self.print_verbose(f'resonv{i}_out = {x.shape}')
            x = x + residual[:, :, (self.kernel_size - 1):]

            self.print_verbose(f'conv{i}_out = {x.shape}')

        self.print_verbose(f'skip_out = {skip.shape}')
        x = F.relu(skip)
        x = F.relu(self.end_conv_1(x))
        self.print_verbose(f'end_conv_1 = {x.shape}')
        x = self.end_conv_2(x)
        self.print_verbose(f'end_conv_2 = {x.shape}')
    ...
    # The original forward method only gives you the last few outputs of the conv op
